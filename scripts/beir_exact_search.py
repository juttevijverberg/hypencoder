'''
using faiss CPU version for retrieval
'''
...
class DenseRetrievalExactSearch(BaseSearch):
    ...
    def search_from_files(
        ...
        corpus_embeddings_files: Union[str, Sequence[str]],
        top_k: int,
        ...
        corpus_embeddings_0 = np.ascontiguousarray(corpus_embeddings_0, dtype=np.float32)
        if score_function == "cos_sim":
            faiss.normalize_L2(corpus_embeddings_0)  # no return value. It normalizes the matrix in place.
        ...
        if score_function == "cos_sim":
            # numpy array normalization
             faiss.normalize_L2(query_embeddings) # no return value. It normalizes the matrix in place.
